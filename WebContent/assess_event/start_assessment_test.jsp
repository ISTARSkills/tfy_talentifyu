...
<link href="<%=baseURL%>assets/css/materialize.min.css?a=1" type="text/css" rel="stylesheet" media="screen,projection">
			...
			$(".submitbtn_q").click(function(){
				...
				var skipping = true;
				 ...
				 $('.istar_question').each(function (index, value) { 
					...
					n = n -q_time;
	                   var id = $(this).attr('id');
	                   opname = $('#'+id).data('optgrp');
	                   
	                   if((qindex == index) && $('#'+id + ' input[name='+opname+']:checked').attr('id')==null) {
	                   	skipping = confirm("Are you sure, you want to skip this question ? ");                   	
	                   }
				...
				if(skipping) {
					...
					console.log(qindex);
				    if(qindex===total_que){	
				    	window.location.href = '<%=baseURL%>lessons/end_of_assessment.jsp';
				    }
				}	
